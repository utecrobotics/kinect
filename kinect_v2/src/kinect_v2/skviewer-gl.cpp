...
SkViewerGl* SkViewerGl::self_ = NULL;
...
SkViewerGl::SkViewerGl(const char* strName)
  : poseUser_(0)
{
...
void SkViewerGl::glutIdle()
{
  glutPostRedisplay();
}


void SkViewerGl::glutDisplay()
{
  SkViewerGl::self_->Display();
}


void SkViewerGl::glutKeyboard(unsigned char key, int x, int y)
{
  SkViewerGl::self_->OnKey(key, x, y);
}


void SkViewerGl::Finalize()
{
...
openni::Status SkViewerGl::Init(int argc, char **argv)
{
  pTexMap_ = NULL;
...
openni::Status SkViewerGl::Run()	//Does not return
{
  glutMainLoop();
...
void SkViewerGl::DrawLimb(const nite::SkeletonJoint& joint1,
...
void SkViewerGl::DrawSkeleton(const nite::UserData& userData)
{
...
void SkViewerGl::Display()
{
  ...
  {
    // Texture map init
    nTexMapX_ = MIN_CHUNKS_SIZE(depthFrame.getVideoMode().getResolutionX(),
                                TEXTURE_SIZE);
    nTexMapY_ = MIN_CHUNKS_SIZE(depthFrame.getVideoMode().getResolutionY(),
                                TEXTURE_SIZE);
...
void SkViewerGl::OnKey(unsigned char key, int /*x*/, int /*y*/)
{
  switch (key)
  {
  case 27:
...
openni::Status SkViewerGl::InitOpenGL(int argc, char **argv)
{
  glutInit(&argc, argv);
...
void SkViewerGl::InitOpenGLHooks()
{
