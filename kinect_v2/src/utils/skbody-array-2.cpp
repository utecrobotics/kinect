...
SkBodyArray2* SkBodyArray2::self_ = NULL;
...
SkBodyArray2::SkBodyArray2(const char* strName,
                               ros::NodeHandle& nh)
  :
  poseUser_(0),
  nh_(nh)
{
  ...
  if (SHOW_MARKERS)
  {
    ...
    {
      markers_[i] = new FrameMarker(nh_);
    }
...
openni::Status SkBodyArray2::Init(int argc, char **argv)
{
  pTexMap_ = NULL;
...
void SkBodyArray2::show_marker(const nite::SkeletonJoint& joint,
...
void SkBodyArray2::DrawLimb(const nite::SkeletonJoint& joint1,
...
void SkBodyArray2::DrawSkeleton(const nite::UserData& userData)
{
  if (SHOW_MARKERS)
    lines_->reset();
