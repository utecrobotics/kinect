#include <kinect_v2/frame-marker.hpp>
  ...
  : Marker(nh)
{
  ...
  markerx_.ns = "k_frame_markers";
  markery_.ns = "k_frame_markers";
  markerz_.ns = "k_frame_markers";
  markerx_.id = id_++;
  markery_.id = id_++;
  markerz_.id = id_++;
...
void FrameMarker::setPose(const Eigen::VectorXd& pose)
{
  ...
  if (pose.size()==7)
  {
    ...
    q = quaternionMult(pose.tail(4), q1);
...
Eigen::Vector4d FrameMarker::quaternionMult(const Eigen::Vector4d& q1,
